...
$stmt = $conn->prepare("
    SELECT m.*, 
           ...
           asn.name AS admin_student_name
    FROM messages m
    LEFT JOIN faculty f ON (m.sender_id = f.id AND m.sender_type = 'faculty') OR (m.receiver_id = f.id AND m.receiver_type = 'faculty')
    LEFT JOIN students s ON (m.sender_id = s.id AND m.sender_type = 'student') OR (m.receiver_id = s.id AND m.receiver_type = 'student')
    LEFT JOIN faculty af ON (m.receiver_id = af.id AND m.receiver_type = 'faculty') OR (m.sender_id = af.id AND m.sender_type = 'faculty')
    LEFT JOIN students asn ON (m.receiver_id = asn.id AND m.receiver_type = 'student') OR (m.sender_id = asn.id AND m.sender_type = 'student')
...
if ($result->num_rows > 0) {
    ...
    // Mark as read if admin is receiver
    if ($message['receiver_id'] == $admin_id && $message['receiver_type'] == 'admin' && !$message['is_read']) {
...
if ($message['sender_type'] === 'admin') {
    ...
    $sender_name = $message['faculty_name'] ? $message['faculty_name'] : $message['student_name'];
}

if ($message['receiver_type'] === 'admin') {
    $receiver_name = 'Administrator';
} else {
    $receiver_name = $message['admin_faculty_name'] ? $message['admin_faculty_name'] : $message['admin_student_name'];
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="flex-1 flex flex-col overflow-y-auto">
        ...
        <div class="p-6 flex-1">
            <div class="bg-white rounded-lg shadow max-w-4xl mx-auto">
                <div class="flex justify-between items-center px-6 py-4 border-b">
                    ...
                    <div>
                        <a href="admin_messages.php?tab=<?= $tab ?>" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition flex items-center">
                ...
                <div class="p-6">
                    <?php if ($message): ?>
                        <div class="message-header pb-6 mb-6">
                            <div class="flex justify-between items-start">
                                <div class="flex items-start">
                                    ...
                                    <?php elseif ($message['sender_type'] === 'faculty'): ?>
                                        <div class="avatar faculty-avatar"><?= substr($sender_name, 0, 1) ?></div>
                                    <?php else: ?>
                                        <div class="avatar student-avatar"><?= substr($sender_name, 0, 1) ?></div>
                                    <?php endif; ?>
                                ...
                                <div class="text-right">
                                    ...
                                    <?php if ($message['is_read'] && $message['receiver_type'] === 'admin'): ?>
                        ...
                        <div class="flex justify-between items-center border-t pt-4">
                            ...
                            <div>
                                <form method="POST" action="admin_messages.php" class="inline">
        ...
        onclick="return confirm('Are you sure you want to delete this message?');">
                                ...
                                <?php if ($message['sender_id'] == $admin_id): ?>
                                    <a href="compose_message.php?reply=<?= $message['id'] ?>" 
                    ...
                    <?php else: ?>
                        <div class="text-center py-12">
                            ...
                            <a href="admin_messages.php" class="inline-block mt-6 bg-itu-primary text-white px-6 py-2 rounded-lg">
                                Back to Messages
                            </a>
                        </div>
                    <?php endif; ?>
