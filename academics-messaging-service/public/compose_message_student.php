...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $receiver_type = strtolower(trim($_POST['receiver_type']));
    
    // Get actual admin ID from database if sending to admin
    if ($receiver_type === 'admin') {
        $admin_result = $conn->query("SELECT id FROM admins LIMIT 1");
        if ($admin_result && $admin_result->num_rows > 0) {
            $admin_row = $admin_result->fetch_assoc();
            $receiver_id = (int)$admin_row['id'];
        } else {
 ...
 // Admin ID fixed to 1
    ...
    if (empty($subject) || empty($content)) {
        ...
        $stmt = $conn->prepare("
            ...
            VALUES (?, 'student', ?, ?, ?, ?)
        ");
        $stmt->bind_param("iisss", $student_id, $receiver_id, $receiver_type, $subject, $content);
...
<body class="flex h-screen bg-gray-100 overflow-hidden">
            ...
            <div class="bg-white rounded-lg shadow fade-in max-w-4xl mx-auto">
                <div class="flex justify-between items-center px-6 py-4 border-b">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Send a message to faculty or administrators</p>
                    </div>
                    <div>
                        <a href="student_messages.php" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition flex items-center">
    ...
    <script>
        // Toggle between faculty and admin recipients
        ...
        // Auto-select first faculty by default
