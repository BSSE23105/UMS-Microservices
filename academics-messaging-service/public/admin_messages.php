...
function make_initials($name): string {
    ...
    if (count($parts) > 1) {
        ...
        // second letter of same word if available
...
if (isset($_POST['delete_message'])) {
    ...
    header("Location: /academics/admin_messages.php?tab=$tab");
    exit;
}

// Inbox (case-normalize sender/receiver types)
$stmtInbox = $conn->prepare("
    SELECT m.*,
           f.name AS faculty_name,
           s.name AS student_name
    FROM messages m
    LEFT JOIN faculty  f ON m.sender_id = f.id AND LOWER(m.sender_type) = 'faculty'
    LEFT JOIN students s ON m.sender_id = s.id AND LOWER(m.sender_type) = 'student'
    WHERE m.receiver_id = ?
      AND LOWER(m.receiver_type) = 'admin'
    ORDER BY m.sent_at DESC
");
$stmtInbox->bind_param("i", $admin_id);
$stmtInbox->execute();
$inbox = $stmtInbox->get_result(); // requires mysqlnd [web:7]
...
$stmtSent = $conn->prepare("
    SELECT m.*,
           ...
           s.name AS student_name
    FROM messages m
    LEFT JOIN faculty  f ON m.receiver_id = f.id AND LOWER(m.receiver_type) = 'faculty'
    LEFT JOIN students s ON m.receiver_id = s.id AND LOWER(m.receiver_type) = 'student'
    WHERE m.sender_id = ?
      AND LOWER(m.sender_type) = 'admin'
    ORDER BY m.sent_at DESC
");
$stmtSent->bind_param("i", $admin_id);
$stmtSent->execute();
$sent = $stmtSent->get_result(); // requires mysqlnd [web:7]
...
$stmtUnread = $conn->prepare("
    ...
    FROM messages
    WHERE receiver_id = ?
      AND LOWER(receiver_type) = 'admin'
      AND is_read = 0
");
$stmtUnread->bind_param("i", $admin_id);
$stmtUnread->execute();
$unread_result = $stmtUnread->get_result(); // requires mysqlnd [web:7]
$unread_count = (int)($unread_result->fetch_assoc()['unread_count'] ?? 0);
...
<head>
    ...
    <title>Admin Messages | UMS</title>
    ...
    <div class="sidebar w-64 bg-itu-primary text-white flex flex-col">
        ...
        <div class="flex-1 py-4">
            <nav>
                ...
                <a href="/courses/courses.php" class="block py-3 px-6 hover:bg-itu-secondary">
                    <i class="fas fa-book mr-3"></i>Courses
                </a>
                <a href="schedule.php" class="block py-3 px-6 hover:bg-itu-secondary">
                    <i class="fas fa-calendar-alt mr-3"></i>Schedules
                </a>
                <a href="admin_messages.php" class="block py-3 px-6 bg-itu-secondary">
                    <i class="fas fa-envelope mr-3"></i>Messages
                    <?php if ($unread_count > 0): ?>
                        <span class="bg-red-500 text-white text-xs rounded-full px-2 py-1 ml-2"><?= $unread_count ?></span>
                    <?php endif; ?>
                </a>
            ...
            <div class="bg-white rounded-lg shadow fade-in">
                ...
                <div class="px-6 py-4 border-b flex justify-between items-center">
                    ...
                    <div class="flex space-x-2">
                        <a href="compose_message.php" class="bg-itu-primary hover:bg-itu-secondary text-white px-4 py-2 rounded-lg flex items-center">
...
if ($sender_type === 'faculty') {
    $name = $message['faculty_name'] ?? 'Unknown Faculty';
    $avatar_class = 'faculty-avatar';
} elseif ($sender_type === 'student') {
    $name = $message['student_name'] ?? 'Unknown Student';
    $avatar_class = 'student-avatar';
...
$initials = make_initials($name);
                                ...
                                <tr class="message-card <?= $is_unread ? 'unread' : '' ?>">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="view_messages.php?id=<?= $message['id'] ?>&tab=inbox" class="text-itu-primary hover:text-itu-secondary mr-3">
                        ...
                        <tbody id="sent-container" class="bg-white divide-y divide-gray-200 <?= $active_tab === 'sent' ? '' : 'hidden' ?>">
                            <?php if ($sent->num_rows > 0): ?>
                                <?php while ($message = $sent->fetch_assoc()): 
                                    $receiver_type = strtolower((string)$message['receiver_type']);

if ($receiver_type === 'faculty') {
    $name = $message['faculty_name'] ?? 'Unknown Faculty';
    $avatar_class = 'faculty-avatar';
} elseif ($receiver_type === 'student') {
    $name = $message['student_name'] ?? 'Unknown Student';
    $avatar_class = 'student-avatar';
...
$initials = make_initials($name);
                                ...
                                <tr class="message-card">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="view_messages.php?id=<?= $message['id'] ?>&tab=sent" class="text-itu-primary hover:text-itu-secondary mr-3">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form method="POST" class="inline">
                                            <input type="hidden" name="message_id" value="<?= $message['id'] ?>">
    ...
    <script>
        ...
        document.getElementById('inbox-tab').addEventListener('click', function() {
            ...
            history.replaceState(null, null, 'admin_messages.php?tab=inbox');
        });
        ...
        document.getElementById('sent-tab').addEventListener('click', function() {
            ...
            // Update URL without reloading
            history.replaceState(null, null, 'admin_messages.php?tab=sent');
        });
