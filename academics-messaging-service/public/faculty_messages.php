...
if (isset($_POST['delete_message'])) {
    ...
    header("Location: faculty_messages.php?tab=$tab");
    exit;
}
...
$inbox = $conn->query("
    ...
    WHERE m.receiver_id = $faculty_id AND m.receiver_type = 'faculty'
...
$sent = $conn->query("
    ...
    WHERE m.sender_id = $faculty_id AND m.sender_type = 'faculty'
    ORDER BY m.sent_at DESC
");
...
<head>
    ...
    <title>Faculty Messages | UMS</title>
...
<body class="flex h-screen bg-gray-100 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-y-auto">
        ...
        <header class="bg-white shadow">
            <div class="flex justify-between items-center px-6 py-4">
                <div>
                    <h1 class="text-xl font-semibold text-gray-800">Faculty Dashboard</h1>
                </div>
            ...
            <div class="bg-white rounded-lg shadow fade-in">
                ...
                <div class="px-6 py-4 border-b flex justify-between items-center">
                    ...
                    <div class="flex space-x-2">
                        <a href="compose_message_faculty.php" class="bg-itu-primary hover:bg-itu-secondary text-white px-4 py-2 rounded-lg flex items-center">
                        ...
                        <tbody id="inbox-container" class="bg-white divide-y divide-gray-200 <?= $active_tab === 'inbox' ? '' : 'hidden' ?>">
                            <?php if ($inbox->num_rows > 0): ?>
                                <?php while ($message = $inbox->fetch_assoc()): 
                                    ...
                                    $name = $sender_type === 'admin' ? $message['admin_name'] : $message['student_name'];
                                    $initials = $sender_type === 'admin' ? 'A' : substr($name, 0, 1);
                                    $avatar_class = $sender_type === 'admin' ? 'admin-avatar' : 'student-avatar';
                                ?>
                                <tr class="message-card <?= $is_unread ? 'unread' : '' ?>">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="view_message_faculty.php?id=<?= $message['id'] ?>&tab=inbox" class="text-itu-primary hover:text-itu-secondary mr-3">
                        ...
                        <tbody id="sent-container" class="bg-white divide-y divide-gray-200 <?= $active_tab === 'sent' ? '' : 'hidden' ?>">
                            <?php if ($sent->num_rows > 0): ?>
                                <?php while ($message = $sent->fetch_assoc()): 
                                    ...
                                    $name = $receiver_type === 'admin' ? $message['admin_name'] : $message['student_name'];
                                    $initials = $receiver_type === 'admin' ? 'A' : substr($name, 0, 1);
                                    $avatar_class = $receiver_type === 'admin' ? 'admin-avatar' : 'student-avatar';
                                ?>
                                <tr class="message-card">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <a href="view_message_faculty.php?id=<?= $message['id'] ?>&tab=sent" class="text-itu-primary hover:text-itu-secondary mr-3">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form method="POST" class="inline">
                                            <input type="hidden" name="message_id" value="<?= $message['id'] ?>">
    ...
    <script>
        ...
        document.getElementById('inbox-tab').addEventListener('click', function() {
            ...
            history.replaceState(null, null, 'faculty_messages.php?tab=inbox');
        });
        ...
        document.getElementById('sent-tab').addEventListener('click', function() {
            ...
            // Update URL without reloading
            history.replaceState(null, null, 'faculty_messages.php?tab=sent');
        });
