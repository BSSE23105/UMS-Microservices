...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($id)) {
        ...
        $stmt = $conn->prepare("INSERT INTO faculty (id, name, email, password, qualification, experience, age, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssiss", $id, $name, $email, $password, $qualification, $experience, $age, $gender);
...
<head>
    ...
    <title>Faculty Registration - University Management</title>
    ...
    <style>
        ...
        * {
        ...
        .container::before {
            ...
            right: 0;
            height: 8px;
            background: linear-gradient(to right, var(--itu-accent), var(--itu-primary));
        }
        .header {
            text-align: center;
            margin-bottom: 30px;
        ...
        .btn-submit {
            background: linear-gradient(to right, var(--itu-accent), #f9b04d);
            color: white;
            border: none;
            padding: 14px;
            border-radius: 8px;
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Register new faculty members into the university system</p>
        </div>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                <label for="id"><i class="fas fa-id-card"></i> Faculty ID</label>
                <i class="fas fa-hashtag input-icon"></i>
                <input type="number" id="id" name="id" min="1" required placeholder="Enter unique faculty ID">
            </div>
            
            <div class="form-group">
                <label for="name"><i class="fas fa-user"></i> Full Name</label>
                <i class="fas fa-signature input-icon"></i>
                <input type="text" id="name" name="name" required placeholder="Enter faculty's full name">
            </div>
            
            <div class="form-group">
                <label for="email"><i class="fas fa-envelope"></i> Email Address</label>
                <i class="fas fa-at input-icon"></i>
                <input type="email" id="email" name="email" required placeholder="Enter faculty's email">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="qualification" name="qualification" required placeholder="Enter highest qualification">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="age" name="age" min="1" required placeholder="Enter faculty member's age">
            </div>
